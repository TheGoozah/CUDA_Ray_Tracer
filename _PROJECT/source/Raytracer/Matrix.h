...
/*=============================================================================*/
// Matrix.h: base Column-Major Matrix struct stored in Row-Major Order for locality + general matrix template functions
/*=============================================================================*/
#pragma once
...
#pragma region MemberAccessOperators
	...
	BOTH_CALLABLE inline const Vector<N, T>& operator[](uint8_t c) const
	{
		assert((c < N) && "ERROR: index of Matrix [] operator is out of bounds!");
		return (*reinterpret_cast<Vector<N, T>*>(data[c]));
	}

	BOTH_CALLABLE inline Vector<N, T>& operator[](uint8_t c)
	{
		assert((c < N) && "ERROR: index of Matrix [] operator is out of bounds!");
		return (*reinterpret_cast<Vector<N, T>*>(data[c]));
	}
